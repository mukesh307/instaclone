...
const ChatPage = () => {
    ...
    const sendMessageHandler = async (receiverId) => {
        try {
            const res = await axios.post(`https://instaclone-54jh.onrender.com/api/v1/message/send/${receiverId}`, { textMessage }, {
                headers: {
    ...
    return (
        <div className='flex flex-col md:flex-row md:ml-[20%] h-screen'>
            <section className='w-full md:w-1/4 my-8'>
                <h1 className='font-bold mb-4 px-3 text-xl'>{user?.username}</h1>
                <hr className='mb-4 border-gray-300' />
                <div className='overflow-y-auto h-[50vh] md:h-[80vh]'>
                    {
            ...
            {
                selectedUser ? (
                    <section className='flex-1 border-t md:border-t-0 md:border-l border-gray-300 flex flex-col h-full'>
